...
export default function Publishers() {
    ...
    const handleFilterChange = (name: any, value: any) => {
        ...
        // handleExportTable(currentPage, pageSize, updatedFilters, searchText);
    };
    ...
    const handleResetFilters = () => {
        ...
        // handleExportTable(currentPage, pageSize, defaultFilters);
        handleReset();
    };
    ...
    const buildParams = (page: number, size: number, filters: any, searchText: string, sortField: string, sortOrder: 'asc' | 'desc') => {
        return {
            page,
            page_size: size,
            search: searchText,
    ...
    const fetchWebsites = async (page: number = 1, size: number = 10, filters: any = {}, searchText: string = '', sortField: string = '', sortOrder: 'asc' | 'desc' = 'asc') => {
        ...
        try {
            const response = await axios.get(`${BASE_URL}publisher-sites/all/`, {
                params: buildParams(page, size, filters, searchText, sortField, sortOrder),
            });
    ...
    const handleExportTable = async (page: number = 1, size: number = 10, filters: any = {}, searchText: string = '', sortField: string = '', sortOrder: 'asc' | 'desc' = 'asc') => {
        ...
        try {
            const response = await axios.get(`${BASE_URL}publisher-sites/all/?export=csv`, {
                params: buildParams(page, size, filters, searchText, sortField, sortOrder),
                headers: {
            ...
            console.error('Failed to export websites:', error);
        }
    };
    ...
    const handlePageChange = (page: any) => {
        ...
        // handleExportTable(page, pageSize, filters, searchText);
    };
    const handlePageSizeChange = (current: any, size: any) => {
        setPageSize(size);
        setCurrentPage(1);
        fetchWebsites(1, size, filters, searchText);
        // handleExportTable(1, size, filters, searchText);
    };
    ...
    const handleSearch = (selectedKeys: any, confirm: () => void, dataIndex: string) => {
        ...
        // handleExportTable(currentPage, pageSize, filters, searchText);
    };
    ...
    const handleReset = (clearFilters?: () => void) => {
        ...
        // handleExportTable(currentPage, pageSize, filters, '');
    };
    ...
    const websiteCont = websites.map((website) => {
        const countries = website.country_detail.map((country: any) => country.name);
        const firstCountry = countries[0]; // Get the first country
        const additionalCount = countries.length - 1; // Count additional countries

        // Format the string based on the count of additional countries
        return additionalCount > 0 ? `${firstCountry} +${additionalCount}` : firstCountry;
    });
    // console.log(websiteCont);
                        ...
                        <div className="table-container shadow-lg rounded-lg bg-white p-5 dark:bg-dark dark:text-white">
                            ...
                            <div className='w-full flex justify-between items-center'>
                                ...
                                <div className=' flex items-center gap-4'>
                                    {profile === 8 &&
                                                                    <button 
                                                                    className='border py-2 px-4 rounded-lg border-primary text-primary font-medium text-base hover:text-white hover:bg-primary smooth'
                                                                    onClick={() => handleExportTable(currentPage, pageSize, filters, searchText)}>Export Table</button>
                                    }
